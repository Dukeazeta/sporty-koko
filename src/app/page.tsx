    ...
    <main className="min-h-screen p-4 md:p-8 max-w-5xl mx-auto bg-[var(--neo-bg)]">
      ...
      <header className="mb-16 text-center relative">
        ...
        <div className="relative z-10 py-8 px-4">
          <h1 className="text-5xl md:text-8xl font-black uppercase tracking-tighter mb-4 text-white drop-shadow-[4px_4px_0px_rgba(0,0,0,1)] stroke-black" style={{ WebkitTextStroke: '3px black' }}>
            SportyKoko
          </h1>
          <div className="inline-block bg-[var(--neo-pink)] border-4 border-black p-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rotate-2">
            <p className="text-xl md:text-2xl font-bold font-mono text-white uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
        {competitions.map((comp, index) => (
          <Link key={comp.id} href={`/competitions/${comp.id}`} className="block group">
            <div className={`neo-box p-8 h-full flex flex-col justify-between bg-white hover:bg-[var(--neo-green)] transition-all ${index % 2 === 0 ? 'rotate-1' : '-rotate-1'} group-hover:rotate-0`}>
              ...
              <div>
                <h2 className="text-4xl font-black uppercase leading-none mb-2 group-hover:text-white group-hover:drop-shadow-[2px_2px_0px_rgba(0,0,0,1)] transition-all">
                  {comp.name}
                </h2>
                <div className="w-full h-1 bg-black my-4"></div>
                <div className="flex items-center justify-between">
                  <span className="font-bold text-lg group-hover:text-white">VIEW MATCHES</span>
                  <IconArrowRight className="w-8 h-8 transform group-hover:translate-x-2 transition-transform" />
                </div>
              </div>
            </div>
          </Link>
        ...
        {competitions.length === 0 && (
          <div className="col-span-full neo-box p-12 text-center bg-gray-100 rotate-1">
            <h3 className="text-3xl font-black uppercase mb-4">No Competitions Found</h3>
            <p className="font-mono text-lg mb-6">The API might be taking a nap.</p>
